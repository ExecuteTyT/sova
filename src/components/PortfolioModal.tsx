  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <motion.div 
        ...
        className="relative w-full max-w-5xl bg-[#1A1A1A] rounded-sm shadow-2xl overflow-hidden flex flex-col z-10 h-[90vh] md:h-auto max-h-[90vh]"
      >
        <button 
          onClick={onClose}
        ...
        <div className="relative h-full flex flex-col w-full">
          <div className="flex-1 overflow-y-auto scrollbar-hide pb-[90px] md:pb-0 md:overflow-hidden flex flex-col md:flex-row">
            {/* Left: Gallery */}
            <div className="w-full md:w-3/5 flex flex-col bg-[#111111]">
          <div className="relative aspect-[4/3] w-full overflow-hidden bg-[#111111]">
            <img src={mainImg} alt={item.title} className="w-full h-full object-cover" data-replace="true" />
          </div>
          <div className="flex gap-3 p-4 overflow-x-auto scrollbar-hide snap-x snap-mandatory bg-[#1A1A1A]">
            {thumbnails.map((thumb, idx) => (
              ...
              >
                <img src={thumb} alt="thumbnail" className="w-full h-full object-cover" data-replace="true" />
              </button>
            ))}
          </div>
        </div>
          ...
          <div className="mb-8">
            <div className="text-sm text-[#A1A1AA] mb-1">Стоимость реализации:</div>
            <div className="text-[28px] font-serif text-white">{item.price}</div>
          </div>

          <div className="mb-8">
            <div className="text-sm text-[#A1A1AA] mb-3">Использованные материалы:</div>
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-[#1A1A1A] border-t border-white/5 p-4 md:hidden z-10">
            <Button 
              fullWidth 
              onClick={() => {
                onClose();
