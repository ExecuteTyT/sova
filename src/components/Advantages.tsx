...
export default function Advantages() {
  ...
  return (
    <section id="advantages" className="py-24 bg-bg-alt relative overflow-hidden">
      {/* Decorative blob */}
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-accent/6 rounded-full blur-[100px] pointer-events-none translate-x-1/2 -translate-y-1/4" />
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-[55%_45%] gap-16 items-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              {advantages.map((adv, index) => (
                <motion.div key={index} variants={fadeUp} className="flex flex-col gap-4">
                  <div className="w-14 h-14 rounded-sm bg-accent-light border border-accent/20 flex items-center justify-center shadow-custom">
                    <adv.icon strokeWidth={1.5} size={22} className="text-accent-deep" />
                  </div>
                  <div>
          ...
          <motion.div
            ...
            className="relative"
          >
            <div className="relative aspect-[4/5] overflow-hidden rounded-sm shadow-custom-lg">
              <img
